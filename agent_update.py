...
full_translation 必须是对 original_text 的 1:1 精准翻译。严禁增删原文信息，严禁添加原文中不存在的背景解释。

[极高优先级 - source_url 规范]
source_url 只能从 raw_tweets.txt 中对应条目的「链接:」字段直接复制。如果该条目没有「链接:」字段，source_url 必须为空字符串 ""。绝对不要编造、猜测或推断任何 URL。宁可留空也不要提供虚假链接。
source_name 为信息来源的简短名称（从 raw_tweets.txt 的「来源:」字段提取）。
"""
...
def validate_and_merge(new_data: dict, output_path: str, max_articles: int = 200) -> None:
    """校验新数据（1-4篇文章），与旧数据合并，截断后写入 data.json"""
    # 校验新数据结构
    if "articles" not in new_data:
        raise ValueError("JSON 缺少 articles 字段")

    new_articles = new_data["articles"]
    if not isinstance(new_articles, list) or len(new_articles) < 1:
        raise ValueError("articles 至少包含 1 篇文章")
